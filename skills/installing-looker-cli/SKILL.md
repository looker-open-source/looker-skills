...
-   Privileges to write to local binary directories (e.g. `/usr/local/bin` or `~/.local/bin`).
...
1.  **Retrieve latest release tag**: Query the GitHub API or check the releases page (`https://github.com/looker-open-source/looker-cli/releases`) to identify the latest version tag (e.g., `v1.0.0`).
2.  **Download and install**:
    Run the following sequence to download, rename, and move the binary to your local bin path:
...
1.  **Download**: Download the latest Windows binary (e.g. using PowerShell):
    ...
    ```
2.  **Add to Path**: Move `looker-cli.exe` to a directory in your system `%PATH%` (or add its directory to the Environment Variables).
